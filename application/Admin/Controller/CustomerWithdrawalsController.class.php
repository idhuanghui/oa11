...
/**
 * 提现查询
 * Class CustomerWithdrawalsController
 * @package Admin\Controller
 */
class CustomerWithdrawalsController extends AdminbaseController {
    ...
    function index() {
        ...
        if (session('ADMIN_ID') == 1 || session('roid') == '2') { //超级管理员显示所有的
            ...
            $salesRes = customer_inquiry(C('INTERFACE_URL_TX'),$users_id,$page_size,$p,$customer,$begindate,$enddate);        //var_dump($salesRes);
            ...
            if ($_POST['explode'] == '1') {
                $salesRes = array();
                $salesRes = customer_inquiry(C('INTERFACE_URL_TX'),$users_id,$count,1,$customer,$begindate,$enddate);        //var_dump($salesRes);
            }
            ...
            $salesRes = customer_inquiry(C('INTERFACE_URL_TX'),session('hyd_id'),$page_size,$p,$customer,$begindate,$enddate);        //var_dump($salesRes);
            ...
            if ($_POST['explode'] == '1') {
                $salesRes = array();
                $salesRes = customer_inquiry(C('INTERFACE_URL_TX'),session('hyd_id'),$count,1,$customer,$begindate,$enddate);        //var_dump($salesRes);
            }
            foreach ($salesRes['list'] as $r) {
    ...
    function get_realname(){
            if($_POST['keywords']){
                    $result_users = $this->users_obj->field('user_realname as keywords')
                            ->where("user_realname like '%".$_POST['keywords']."%'")
                            ->select();
                    if($result_users){
                    ...
                    //echo "[{'keywords':'".$_POST['keywords']."'},{'keywords':'".$_POST['keywords']."'}]";                
            }
    }
    ...
    private function _sales_export($sales = array()){
            ...
            foreach ($sales as $k=>$v){
                    $data[$k]['addtime']	= date('Y-m-d H:i:s', $v['addtime']); //提现时间
                    ...
                    $data[$k]['reg_time']	= $v['reg_time'];//注册时间
                    $data[$k]['remark']         = $v['remark'];//提现方式
                    $data[$k]['bank']	= $v['bank'];//到账银行
                    $data[$k]['total']	= $v['total'];//操作金额    
                    $data[$k]['fee']	= $v['fee'];//手续费扣除
                    
                    $data[$k]['credited']	= $v['credited'];//到账金额
