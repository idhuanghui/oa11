;(function () {
            ...
            $('button.J_ajax_submit_btn').on('click', function (e) {
                ...
                form.ajaxSubmit({
                    ...
                    success: function (data, statusText, xhr, $form) {
                        ...
                        //按钮文案、状态修改
                        btn.removeClass('disabled').text(text.replace('中...', '')).parent().find('li').remove();
                        if (data.state === 'success') {
                            $('<li class="tips_success">' + data.info + '</li>').prependTo(btn.parent()).fadeIn('slow').delay(1000).fadeOut(function () {
                            });
                        } else if (data.state === 'fail') {
                            $('<li class="tips_error">' + data.info + '</li>').prependTo(btn.parent()).fadeIn('fast');
    ...
    //离职操作，离职后刷新页面
    if ($('a.J_ajax_quit').length) {
        Wind.use('artDialog', function () {
            $('.J_ajax_quit').on('click', function (e) {
                e.preventDefault();
                var $_this = this,
                    $this = $($_this),
                    href = $this.prop('href'),
    ...
    //取消入职，取消后刷新页面
    if ($('a.J_ajax_cancel').length) {
        Wind.use('artDialog', function () {
            $('.J_ajax_cancel').on('click', function (e) {
                e.preventDefault();
                var $_this = this,
                    $this = $($_this),
                    href = $this.prop('href'),
